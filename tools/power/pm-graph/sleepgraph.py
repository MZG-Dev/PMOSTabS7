...
class SystemValues(object):
	...
	def addKprobes(self, output=False):
		...
		if output:
			...
			linesack = old_div((len(check.split('\n')) - 1), 2)
	...
	def initFtrace(self):
		...
		if self.bufsize > 0:
			...
			tgtsize = 65536
		while not self.fsetVal('%d' % (old_div(tgtsize, cpus)), 'buffer_size_kb'):
			# if the size failed to set, lower it and keep trying
			tgtsize -= 65536
			if tgtsize < 65536:
				tgtsize = int(self.fgetVal('buffer_size_kb')) * cpus
				break
		print(('Setting trace buffers to %d kB (%d kB per cpu)' % (tgtsize, old_div(tgtsize,cpus))))
	...
	def selectTimelineDevices(self, widfmt, tTotal, mindevlen):
		...
		for phase in self.dmesg:
			...
			for dev in list:
				...
				width = widfmt % (old_div(((list[dev]['end']-list[dev]['start'])*100),tTotal))
...
class Timeline(object):
	...
	def calcTotalRows(self):
		...
		for t, p in standardphases:
			for i in sorted(self.rowheight[t][p]):
				self.rowheight[t][p][i] = old_div(self.bodyH,len(self.rowlines[t][p]))
	...
	def createTimeScale(self, m0, mMax, tTotal, mode):
		...
		if(tTotal > 4):
			tS = 1
		divTotal = int(old_div(mTotal,tS)) + 1
		divEdge = old_div((mTotal - tS*(divTotal-1))*100,mTotal)
		for i in range(divTotal):
			htmlline = ''
			if(mode == 'suspend'):
				pos = '%0.3f' % (100 - (old_div((float(i)*tS*100),mTotal)) - divEdge)
				val = '%0.fms' % (float(i-divTotal+1)*tS*1000)
				if(i == divTotal - 1):
					val = mode
				htmlline = timescale.format(pos, val)
			else:
				pos = '%0.3f' % (100 - (old_div((float(i)*tS*100),mTotal)))
...
def createHTMLSummarySimple(testruns, htmlfile, folder):
	...
	for data in sorted(testruns, key=lambda v:(v['mode'], v['host'], v['kernel'], v['time'])):
		...
		if lastmode and lastmode != mode and num > 0:
			for i in range(2):
				s = sorted(tMed[i])
				list[lastmode]['med'][i] = s[int(old_div(len(s),2))]
				iMed[i] = tMed[i].index(list[lastmode]['med'][i])
			list[lastmode]['avg'] = [old_div(tAvg[0], num), old_div(tAvg[1], num)]
...
def createHTML(testruns, testfail):
	...
	for data in testruns:
		...
		for dir in phases:
			...
			if dir == 'suspend':
				...
				left = '%f' % (old_div(((m0-t0)*100.0),tTotal))
			else:
				m0 = data.tSuspended
				mMax = data.end
				...
				left = '%f' % (old_div((((m0-t0)*100.0)+old_div(sysvals.srgap,2)),tTotal))
			mTotal = mMax - m0
			# if a timeline block is 0 length, skip altogether
			if mTotal == 0:
				continue
			width = '%f' % (old_div(((mTotal*100.0)-old_div(sysvals.srgap,2)),tTotal))
			devtl.html += devtl.html_tblock.format(bname, left, width, devtl.scaleH)
			for b in sorted(phases[dir]):
				# draw the phase color background
				phase = data.dmesg[b]
				length = phase['end']-phase['start']
				left = '%f' % (old_div(((phase['start']-m0)*100.0),mTotal))
				width = '%f' % (old_div((length*100.0),mTotal))
			...
			for e in data.errorinfo[dir]:
				...
				type, t, idx1, idx2 = e
				id = '%d_%d' % (idx1, idx2)
				right = '%f' % (old_div(((mMax-t)*100.0),mTotal))
			...
			for b in sorted(phases[dir]):
				...
				for d in data.tdevlist[b]:
					...
					left = '%f' % (old_div(((dev['start']-m0)*100),mTotal))
					width = '%f' % (old_div(((dev['end']-dev['start'])*100),mTotal))
					...
					if('cpuexec' in dev):
						for t in sorted(dev['cpuexec']):
							...
							if j > 1.0:
								j = 1.0
							height = '%.3f' % (old_div(rowheight,3))
							top = '%.3f' % (rowtop + devtl.scaleH + old_div(2*rowheight,3))
							left = '%f' % (old_div(((start-m0)*100),mTotal))
							width = '%f' % (old_div((end-start)*100,mTotal))
					...
					for e in dev['src']:
						...
						left = '%f' % (old_div(((e.time-m0)*100),mTotal))
						width = '%f' % (old_div(e.length*100,mTotal))
						xtrastyle = ''
						if e.color:
	...
	for data in testruns:
		...
		for b in data.phases:
			phase = data.dmesg[b]
			length = phase['end']-phase['start']
			left = '%.3f' % (old_div(((phase['start']-t0)*100.0),tTotal))
			width = '%.3f' % (old_div((length*100.0),tTotal))
...
def ms2nice(val):
	val = int(val)
	h = old_div(val, 3600000)
	m = (old_div(val, 60000)) % 60
	s = (old_div(val, 1000)) % 60
	if h > 0:
